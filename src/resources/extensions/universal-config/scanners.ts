...
async function scanClaude(projectRoot: string, home: string, tool: ToolInfo): Promise<{ items: DiscoveredItem[]; warnings: string[] }> {
    ...
    // Project-level MCP: .mcp.json (standard), .lsd/mcp.json (local-only), .claude/.mcp.json, or .claude/mcp.json
    for (const relPath of [".mcp.json", ".lsd/mcp.json", ".claude/.mcp.json", ".claude/mcp.json"]) {
