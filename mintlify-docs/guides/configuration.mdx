...
- `.lsd/mcp.json` — local-only config
...
Optional cheap-model alias used by bundled budget-oriented subagents.
...
The bundled `scout` subagent resolves `model: $budget_model`, so setting this preference makes scout use the cheaper model automatically. If omitted, scout falls back to the current session/default model.
