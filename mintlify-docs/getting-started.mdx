...
To connect GSD to local or external MCP servers, add project-local config in `.mcp.json` or `.lsd/mcp.json`. See [configuration](/guides/configuration) for examples. Use `/gsd mcp` to verify connectivity.
